...
struct LoadLoadOpStore {
  LoadLoadOpStore(std::vector<llvm::Value*> inputs, llvm::Instruction* op,
                  std::vector<llvm::Value*> stores)
      : inputs(inputs), op(op), stores(stores){};

  friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os,
                                       const LoadLoadOpStore& llos) {
    os << "Inputs:\n";
 ...
 */
struct OpStore {
  OpStore(std::vector<llvm::Value*> inputs, llvm::Instruction* op,
          std::vector<llvm::Value*> stores)
      : inputs(inputs), op(op), stores(stores){};
...
class FindLdLdOpSt : public llvm::ModulePass {
 public:
  ...
  void print(llvm::raw_ostream& os, const llvm::Module* m) const override {
    ...
    for (LoadLoadOpStore llos : lloss) histogram[llos.op->getOpcode()]++;
    ...
    os << "\n";
    os << "OP-ST DATA (op-st = group of instructions that was nearly a "
          "ld-ld-op-st)\n";
    os << "\n";
...
void findLoadLoadOpStore(llvm::Function& function,
  ...
  for (llvm::inst_iterator I = inst_begin(function), E = inst_end(function);
       I != E; ++I) {
    worklist.push_back(&*I);
  }

  for (std::vector<llvm::Instruction*>::iterator iter = worklist.begin();
    ...
    for (llvm::Use& use : instr->operands()) {
      ...
      // First, before all else, check to see if this thing is used only by the
      // op. If so, then we're good to replace it. If not, then it could still
      // be replaced, but requires a more complicated analysis.
      ...
      if (use.get()->hasNUsesOrMore(2)) {
        ...
        llvm::errs() << "One of the operands is used by at least one other "
                        "instruction. Can't get rid of this operand.\n";
        break;
      }

      if (llvm::Instruction* useInstr =
              llvm::dyn_cast<llvm::Instruction>(use.get())) {
        if (useInstr->getOpcode() == llvm::Instruction::Load) {
        } else {
          llvm::errs()
              << "Found an operand that is an instruction other than Load: "
              << *use.get() << "\n";
        ...
        llvm::errs() << "Found an operand that is of an unsupported type: "
                     << use->getType() << "\n";
        allLoadsOrImmediates = false;
        allLoads = false;
        break;
     ...
     * Though I'm not gonna argue that it's better. It's definitely not more
    readable. std::vector<bool> usersAreStores;
    std::transform(instr->users().begin(), instr->users().end(),
                    usersAreStores.begin(),
                    [](llvm::User* user){return
    llvm::dyn_cast<llvm::StoreInst>(user);});
    std::reduce(usersAreStores.begin(), usersAreStores.end(), true,
    std::multiplies<bool>);
    */
    bool allStores = true;
    ...
    if (allLoads && allStores && allOperandsOnlyUsedByInstr) {
      ...
      foundLlos.emplace_back(
          std::vector<llvm::Value*>(instr->operands().begin(),
                                    instr->operands().end()),
          instr,
          std::vector<llvm::Value*>(instr->users().begin(),
                                    instr->users().end()));
    }
    ...
    // instruction(s) using the result of op.
      ...
      foundOs.emplace_back(std::vector<llvm::Value*>(instr->operands().begin(),
                                                     instr->operands().end()),
                           instr,
                           std::vector<llvm::Value*>(instr->users().begin(),
                                                     instr->users().end()));
  ...
  // TODO i need to filter out by llos's whose inputs are only used by the op in
  // the llos. that is, if i'm going to replace l-l-o-s with one instruction in
  // memory, i can't just throw out loads whose values are used elsewhere.
}
...
bool replaceLoadLoadOpStore(std::vector<LoadLoadOpStore> loadLoadOpStores) {
  ...
  for (LoadLoadOpStore llos : loadLoadOpStores) {
    ...
    llvm::Function* cacheOperandA = llvm::Intrinsic::getDeclaration(
        module, llvm::Intrinsic::ID::cache_operand_a);
    llvm::Function* cacheOperandB = llvm::Intrinsic::getDeclaration(
        module, llvm::Intrinsic::ID::cache_operand_b);
    llvm::Function* cacheDest = llvm::Intrinsic::getDeclaration(
        module, llvm::Intrinsic::ID::cache_dest);

    llvm::CallInst* callCacheOperandA = llvm::CallInst::Create(
        cacheOperandA, llvm::ArrayRef<llvm::Value*>(llos.inputs[0]));
    llvm::CallInst* callCacheOperandB = llvm::CallInst::Create(
        cacheOperandB, llvm::ArrayRef<llvm::Value*>(llos.inputs[1]));
    llvm::CallInst* callCacheDest = llvm::CallInst::Create(
        cacheDest, llvm::ArrayRef<llvm::Value*>(llos.stores[0]));

    llos.op->getParent()->getInstList().insert(
        llvm::BasicBlock::iterator(llos.op), callCacheOperandA);
    llos.op->getParent()->getInstList().insert(
        llvm::BasicBlock::iterator(llos.op), callCacheOperandB);
    ...
    for (llvm::Value* output : llos.stores)
      if (llvm::Instruction* inst = llvm::dyn_cast<llvm::Instruction>(output))
        inst->eraseFromParent();
  }
...
static llvm::RegisterPass<FindLdLdOpSt> a(
    "find-ld-ld-op-st", "Find and print ld-ld-op-store patterns",
    true,  // CFG only (TODO what is this?)
    true   // analysis pass
);

char ReplaceLdLdOpSt::ID = 1;
static llvm::RegisterPass<ReplaceLdLdOpSt> b(
    "replace-ld-ld-op-st", "Find and replace ld-ld-op-store patterns");
